...
import styles from './StandbylistWebPart.module.scss';
import {
  SPHttpClient,
...
private LoadListItems(): void{
  ...
  if (location.pathname.split("/")[4].replace(".aspx","") == "%E3%81%9D%E3%81%AE%E4%BB%96"){
    ...
    let url: string = this.context.pageContext.web.absoluteUrl + `/_api/web/lists/getbytitle('全エンジニアリスト')/items?$filter=OData__x6240__x5c5e__x4f1a__x793e_ eq '${location.pathname.split("/")[4].replace(".aspx","")}'&$top=1000`;
    this.GetListItems(url).then((response)=>{
      if(!response.value)
      {
      }
