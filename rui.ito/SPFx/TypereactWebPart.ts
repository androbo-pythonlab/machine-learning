...
export default class TypereactWebPart extends BaseClientSideWebPart <ITypereactWebPartProps> {
  ...
  public render(): void {
    ...
    let url: string = this.context.pageContext.web.absoluteUrl + `/_api/web/lists/getbytitle('全エンジニアリスト')/items?$filter=(${status}%20eq%20null%20or%20${status}%20eq%20%27待機%27%20or%20${status}%20eq%20%27稼働中%27)`;  
  ...
  private RenderListItems(items: spListItem[]): standbynum{  
    ...
    */
    ...
    let tempdate = item.OData__x5951__x7d04__x66f4__x65b0__x65.split('T')[0].replace(/-/g,'/');;
